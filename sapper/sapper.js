...
//console.log(arr_count_td);
    ...
    $(table_main).on('click', 'td', function () {
        ...
        if (obj_all_td[this_number]['bomb']) {
            ...
            showMessage('<center>Пойман за руку, как дешёвка...<br></center>Время: ' + $('div#sapper_time').html()+'<br><img src="images/bomb.png"/>', 'red')
			var sound = Math.floor(Math.random()*3);
			switch (sound) {
  case 0:
    var audio = new Audio('sounds/loos.mp3');
			audio.play();
    break;
  case 1:
    var audio = new Audio('sounds/shval.mp3');
			audio.play();
    break;
  case 2:
    var audio = new Audio('sounds/destroy.mp3');
			audio.play();
    break;
  default:
    alert( "Смэрт"+sound );
